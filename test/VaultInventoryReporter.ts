...
const hashItem = (item: Item) => {
    const tokenId = item.itemType == 2 ? 0 : item.tokenId;

    const types = ["address", "uint256"];
    const values = [item.tokenAddress, tokenId];
...
describe("VaultInventoryReporter", () => {
    ...
    describe("Inventory Operations", () => {
        describe("add", () => {
            ...
            it("should not report duplicate ERC721 entries", async () => {
                const { reporter, user, vault } = ctx;

                const items = [defaultItems[1]]; // 721 item only
                ...
                // Enumeration should return length 1
                let reportedItems = await reporter.enumerate(vault.address);
                expect(reportedItems.length).to.eq(1);

                // Attempt to add the same item again - enumeration should be length 1
                ...
                reportedItems = await reporter.enumerate(vault.address);
                expect(reportedItems.length).to.eq(1);

                // Attempt to add the same item again, with a different amount - still length 1
                const modifiedItems = [Object.assign(defaultItems[1], { tokenAmount: 100 })];

                await expect(
                    reporter.connect(user).add(vault.address, modifiedItems)
                ).to.emit(reporter, "Add")
                    .withArgs(vault.address, user.address, hashItem(items[0]));
            ...
            it("should update amounts for an ERC20 added a second time", async () => {
                const { reporter, user, vault } = ctx;

                const items = [defaultItems[0]]; // 20 item only
                ...
                let reportedItems = await reporter.enumerate(vault.address);
                expect(reportedItems.length).to.eq(1);
                expect(reportedItems[0].tokenAmount).to.eq(items[0].tokenAmount);

                // Attempt to add the same item again - enumeration should be length 1, but return
                // different data
                const modifiedItems: Item[] = [Object.assign(defaultItems[0], { tokenAmount: 555 })];

                await expect(
                    reporter.connect(user).add(vault.address, items)
                ).to.emit(reporter, "Add")
                    .withArgs(vault.address, user.address, hashItem(modifiedItems[0]));

                reportedItems = await reporter.enumerate(vault.address);
                expect(reportedItems.length).to.eq(1);
                expect(reportedItems[0].tokenAmount).to.eq(modifiedItems[0].tokenAmount);
                expect(reportedItems[0].tokenId).to.eq(0);

                // Attempt to add the same item again, with a different tokenId - still length 1,
                // since tokenId is ignored for ERC20s
                modifiedItems[0].tokenId = 100;
            ...
            it("should update amounts for an ERC1155 of the same token ID added a second time", async () => {
                const { reporter, user, vault, mockERC1155 } = ctx;

                const items = [defaultItems[2]]; // 1155 item only
                ...
                // Attempt to add the same item again - enumeration should be length 1, but return
                // different data
                const modifiedItems: Item[] = [Object.assign({}, defaultItems[2], { tokenAmount: 5 })];
                ...
                // Attempt to add the same item again, with a different tokenId - for ERC1155,
                // this should now become length 2
                const id1155_2 = await mintERC1155(mockERC1155, vault.address, BigNumber.from(100));

                const modifiedItems2: Item[] = [Object.assign({}, defaultItems[2], { tokenId: id1155_2, tokenAmount: 100 })];

                await expect(
                    reporter.connect(user).add(vault.address, modifiedItems2)
                ).to.emit(reporter, "Add")
                    .withArgs(vault.address, user.address, hashItem(modifiedItems2[0]));

                reportedItems = await reporter.enumerate(vault.address);
                expect(reportedItems.length).to.eq(2);
                expect(reportedItems[0].tokenAmount).to.eq(modifiedItems[0].tokenAmount);
                expect(reportedItems[0].tokenId).to.eq(modifiedItems[0].tokenId);
                expect(reportedItems[1].tokenAmount).to.eq(modifiedItems2[0].tokenAmount);
                expect(reportedItems[1].tokenId).to.eq(modifiedItems2[0].tokenId);
            });
        });
